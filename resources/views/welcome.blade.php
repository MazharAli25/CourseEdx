...
@section('main-content')
    ...
    <section>
        <div class="swiper heroSwiper h-[75vh]">
            @isset($sliders)
                @if (empty($sliders) == false)
                    <div class="swiper-wrapper">

                        @forelse ($sliders as $slider)
                            <div class="swiper-slide relative">
                                <img src="{{ asset($slider->image) }}" class="absolute inset-0 w-full h-full object-cover" />

                                <div class="absolute inset-0 bg-gradient-to-r from-black/70 to-black/30"></div>

                                <div class="relative max-w-7xl mx-auto h-full flex items-center px-6">
                                    <div class="text-white max-w-xl">
                                        <h1 class="text-5xl ml-4 font-extrabold leading-tight">
                                            {{ $slider->title ?? 'Build Your Future' }}
                                        </h1>
                                        <p class="mt-4 ml-4 text-gray-200">
                                            {{ $slider->description ?? 'Learn from industry experts' }}
                                        </p>
                                    </div>
                                </div>
                            </div>
                        @empty
                            {{-- FALLBACK SLIDES --}}
                            @include('partials.default-slides')
                        @endforelse
